...
- **배포 모델 전환 완료**: 구 direct-upload(wrangler) → **CF git 프로젝트 `earth2guide-web`**(GitHub 연결).
  ...
  - **배포 트리거 = git push 자동배포**(네이티브 webhook). ✅ 2026-06-15 검증: 최근 배포 3건 전부 `github:push` — push만으로 CF가 자동 빌드·배포. CF API 트리거(`POST .../deployments`)는 이제 **선택적 폴백**. 세션14 'webhook 미작동'은 git 프로젝트 프로비저닝 지연이었고 컷오버 직후 정상화.
...
## ★ 목표 — ✅ 달성 (2026-06-15 검증)
**git push = CF 자동 트리거** 최종형 도달. `git push origin main` 하면 CF가 자동 빌드·배포(2단계 API 트리거 불필요).
- 근거: 최근 배포 3건(6/14 22:51 KST~) 전부 `github:push` 트리거. 세션14 'webhook 미작동'은 git 프로젝트 초기 프로비저닝 지연이었고 컷오버 직후 자동 정상화.
- CF API 트리거는 **선택적 폴백**으로만 보존(주간 자동화 STEP5는 이중 안전차원 유지 — push 자동배포와 중복이나 무해).
...
## ⚠️ 운영 주의
1. **배포 = git push 자동**(네이티브 webhook, 2026-06-15 검증). push하면 CF가 자동 빌드·배포. CF API 트리거는 선택적 폴백(webhook 미발화 시에만).
