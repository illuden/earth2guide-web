...
export async function generateStaticParams() {
  // news segment 카테고리만 prerender (official은 /official/[slug] 담당)
  const posts = await getAllPostSlugsWithCategory()
  const slugs = posts.filter((p) => getPostSegment(p.category) === 'news').map((p) => p.slug)
...
export default async function NewsDetailPage({ params }: PageProps) {
  ...
  if (!post) redirect(`/${locale}/news`)
  if (getPostSegment(post.category) === 'official') {
    // 잘못된 segment 접근 — canonical 경로로 영구 이동 (중복 콘텐츠 방지)
    permanentRedirect(`/${locale}/official/${post.slug}`)
  }
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-12">
      <JsonLd
        data={articleLd({
          locale,
          url: `https://earth2guide.com/${locale}/news/${post.slug}`,
          title: post.title,
